...
// addding 3 sec timer and then mic will start recording automatically
import React, { useEffect, useState } from 'react';
import { Image, Text, TouchableOpacity, View } from 'react-native';
import Voice from '@react-native-community/voice';
import Toast from 'react-native-toast-message';
...
const App = () => {
  ...
  const [message, setMessage] = useState('');
  const [originalMessage, setOriginalMessage] = useState('');
  const [countdown, setCountdown] = useState(3); // Added countdown state
  const text = 'Delhi is the capital of India and it is located in north India';
  ...
  const speechResultsHandler = e => {
    console.log('voice event', e);
    let originalMsg = e.value[0];
    let trimmedMsg = originalMsg.toString().replace(/\s+/g, '').toLowerCase();
    setOriginalMessage(originalMsg);
    setMessage(trimmedMsg);
  };
  ...
  return (
    <View style={{ backgroundColor: 'white', flex: 1 }}>
      ...
      <View style={{ justifyContent: 'center', alignItems: 'center', paddingBottom: 20 }}>
        {countdown > 0 ? (
          <Text style={{ fontSize: 20, color: 'blue' }}>Starting in {countdown} seconds...</Text>
        ) : null}
      </View>
      <View
        style={{
